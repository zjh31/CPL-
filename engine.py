...
def train_one_epoch(args, model: torch.nn.Module, data_loader: Iterable,
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        img_data, text_data, target, weight, _ = batch
        # copy to GPU
        ...
        loss_dict = loss_utils.trans_vg_loss(output, target, weight, epoch)
...
@torch.no_grad()
def generate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    model.eval()
    pred_box_list = []
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Eval:'
    for batch in metric_logger.log_every(data_loader, 100, header):
        img_data, text_data, target, _, size = batch
        # copy to GPU
        img_data = img_data.to(device)
        text_data = text_data.to(device)
        target = target.to(device)
        ...
        dwh = (640- torch.stack([width*size[:,2], height*size[:,2], width*size[:,2], height*size[:,2]], dim=1)) // 2
        output = output - dwh
        ratio = size[:,2].unsqueeze(1).repeat(1,4)
        output = output / ratio
