...
def refine_target(batch_pred, batch_target):
    re_target = []
    batch_preds = batch_pred.unsqueeze(1).repeat(1, 10, 1)
    ious = bbox_iou(torch.flatten(batch_preds, 0, 1), torch.flatten(batch_target, 0, 1), x1y1x2y2=False)
    ious = ious.reshape(-1, 10)
    values, indices = torch.max(ious, dim=1)
    for i, index in enumerate(indices):
        if values[i] >= 0.5:
            re_target.append(batch_preds[i][index])
        else:
            re_target.append(batch_target[i][index]*0.3 + batch_preds[i][index] * 0.7)
    #import pdb; pdb.set_trace()
    return torch.stack(re_target, dim=0)


def trans_vg_loss(batch_pred, batch_target, weight, epoch):
    """Compute the losses related to the bounding boxes, 
       including the L1 regression loss and the GIoU loss
    """
    batch_size = batch_pred.shape[0]
    num_boxes = batch_size
    #target = refine_target(batch_pred, batch_target[:, :10, :])
    ...
    losses = {}
    tau = 10000
    weight = weight + (loss_bbox.sum(dim=1).detach() + loss_giou.detach()).unsqueeze(1) * epoch/100
