...
def collate_fn_test(raw_batch):
    ...
    size = torch.tensor(raw_batch[5])
    if len(raw_batch) == 7:
        batch = [img_data, text_data, bbox, raw_batch[5], raw_batch[6]]
    else:
        batch = [img_data, text_data, bbox, size]
    return tuple(batch)

def collate_fn_train(raw_batch):
    raw_batch = list(zip(*raw_batch))
    ...
    text_data = NestedTensor(word_id, word_mask)
    bbox = torch.tensor(raw_batch[4])
    weight = torch.tensor(raw_batch[5])
    size = torch.tensor(raw_batch[6])
    batch = [img_data, text_data, bbox, weight, size]
    return tuple(batch)
